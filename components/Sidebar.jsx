...
const Sidebar = ({ expand, setExpand }) => {
    ...
    return (
        <div className={`flex flex-col justify-between bg-secondary pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'w-64 p-4' : 'md:w-20 w-0'} max-md:overflow-hidden`}>
            <div>
                ...
                <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? 'bg-foreground hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max' : 'group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg'} `}>
                    <Image className={expand ? 'w-6' : 'w-7'} src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='' />
                    <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-back text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
                        New Chat
                        <div className='w-3 h-3 absolute bg-black rotate-45 left-4 bottom-1.5'></div>
                    </div>
                    {expand && <p className='text font-medium text-background'>New Chat</p>}
                </button>
                ...
                <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                    <Image className={expand ? "w-5" : "w-6.5 mx-auto"} src={expand ? assets.phone_icon : assets.phone_icon_dull} alt='' />
                        <div className={`absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
                            <div className='relative w-max bg-black text-white text-sm p-3rounded-1g shadow-lg'>
